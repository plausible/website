...
There is a great data disparity between the two. The biggest issue with the difference in stats is the fact that AWStats inflates the main numbers such as the total unique visitors by more than 100% and the total page views by more than 18 times. 

Looking at the other metrics, it is very clear that the majority of data included in AWStats is made by robots. AWStats tries to isolate and exclude robots. They automatically exclude the "not viewed traffic" which includes "traffic generated by robots, worms, or replies with special HTTP status codes".

Despite these efforts, it seems to miss out on a lot of bot traffic which gets through to the actual stats. And these seem to be issues with server log analysis as a whole rather than issues with AWStats software itself.
...
Server logs are an option for some site owners with basic analytics needs that are happy to do the work to get it all set up and that can live with inaccuracies caused by so many of web visitors being robots. 
